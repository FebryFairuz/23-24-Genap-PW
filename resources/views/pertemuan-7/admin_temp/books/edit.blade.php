    ...
    <div class="card card-flush h-md-100">
        <div class="card-header p-7">
            <h3 class="card-title align-items-start flex-column">
                <span class="card-label fw-bolder fs-1">Form Update</span>
                <span class="text-gray-500 mt-1 fw-semibold fs-6">Please fill up the form with correctly</span>
            </h3>
        </div>
        <div class="card-body">
            <div class="w-50 m-auto">
                <form action="/catalog-books/update/{{$book->id}}/edit" method="post" enctype="multipart/form-data" autocomplete="off">
                    @method("PUT")
                    @csrf
                    ...
                    <div class="form-group mb-5">
                        <label>Author</label>
                        <input type="text" required class="form-control" name="author" value="{{ $book->author }}"/>
                    </div>
                    <div class="form-group mb-5">
                        <label>Sinopsis</label>
                        <input type="text" required class="form-control" name="sinopsis" value="{{ $book->sinopsis }}"/>
                    </div>
                    <div class="form-group mb-5">
                        <label>Story</label>
